...
No início da execução interativa, o script deve limpar a tela (quando seguro no terminal) antes do cabeçalho operacional.

## Cabeçalho operacional

Todo script interativo deve exibir, no início:

- nome do script;
- propósito em uma linha;
- modo (`install` / `uninstall` / `validate` / etc.);
- raiz do repositório detectada.
...
Script sem contrato claro de execução e reversão não deve ser considerado pronto para uso em produção do template.
